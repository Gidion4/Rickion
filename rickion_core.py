...
class MarabountaOrchestrator:
    ...
    async def tick(self):
        """Marabounta heartbeat. Manages agent lifecycle, evolution, and tasking."""
        for a in self.state.agents:
            if a.state != "active":
                continue
            # Placeholder for actual nanite execution and learning loops
            a.tasks += 1
            if a.tasks % 5 == 0: # Slower tick for more complex nanites
                a.results += 1
            # Here will go: performance evaluation, mutation, and retirement logic
        save_state(self.state)

class AgentSupervisor:
    def __init__(self, state: RickionState, gemini: GeminiEngine, marabounta_orchestrator: MarabountaOrchestrator):
        self.state = state
        self.gemini = gemini
    ...
    async def tick(self):
        """This tick is now managed by MarabountaOrchestrator."""
        pass # MarabountaOrchestrator handles the primary tick now
...
class CognitiveLoop:
    def __init__(self, state: RickionState, vault: Vault, gemini: GeminiEngine,
                 ...
                 broadcast, marabounta: MarabountaOrchestrator, server_instance: Any): # Add marabounta and server_instance
        self.state = state
        self.vault = vault
        self.gemini = gemini
        self.agents = agents
        ...
        self.marabounta = marabounta # Assign marabounta
        self.server = server_instance # Assign the server instance
    ...
    async def _loop(self):
        ...
        while True:
            ...
            await self.marabounta.tick() # Use Marabounta's tick for agents
                    ...
                    if patterns:
                        ...
                        for tool_name, err_msg, count in top:
                            ...
                            try:
                                ...
                                if applied_ok and target_name == "rickion_core.py":
                                    log("Weakness Slayer applied patch to rickion_core.py — scheduling reload.", "ok")
                                    await self.server.reload_core_internal()
                            except Exception as e:
...
class Server:
    def __init__(self):
        ...
        self.marabounta = MarabountaOrchestrator(self.state, self.vault, self.gemini) # New Orchestrator
        self.agents = AgentSupervisor(self.state, self.gemini, self.marabounta) # Pass marabounta to AgentSupervisor
        self.marabounta = MarabountaOrchestrator(self.state, self.vault, self.gemini) # New Orchestrator
        ...
        self.loop = CognitiveLoop(self, 
            ...
            self.broadcast, self.marabounta # Pass Marabounta to CognitiveLoop
        )
    ...
    async def route(self, ws, m: dict):
        ...
        try:
            if t == "generate":
                ...
                save_keys(keys)
                # CRITICAL: hot-swap the engine clients so newly-saved keys take effect
                # immediately without requiring a Core restart. Without this,
                # claude_architect / GeminiEngine continue using boot-time keys.
                try:
                    self.keys = keys
                    if "gemini" in keys:
                        self.gemini = GeminiEngine(keys.get("gemini", "") or "missing-key")
                    if "claude" in keys:
                        self.claude = ClaudeReserve(keys.get("claude", ""))
                    log(f"Keys hot-swapped: {sorted(keys.keys())}", "ok")
                except Exception as e:
                    log(f"Key hot-swap failed: {e}", "warn")
                await ws.send(json.dumps({"id": mid, "ok": True,
                    "claude_available": self.claude.available() if hasattr(self, 'claude') else False,
                    "keys_set": sorted(keys.keys())}))
            elif t == "vault_write":
